<x-layout-kader :selectedKader='$selectedKader'>
    ...
    <body class="bg-[#f4fcf7]">
        ...
        document.getElementById('default-search').addEventListener('input', function () {
                ...
                // AJAX Request
                fetch(`{{ route('kader.presensi_bayi.searchKegiatan') }}?search=${encodeURIComponent(search)}`, {
                    headers: {
                    ...
                    .then(data => {
                        ...
                        if (data.jadwals.length > 0) {
                            const presensiRoute = @json(route('kader.cek_presensi', ['id_kegiatan' => ':id'])); // Placeholder ':id'
